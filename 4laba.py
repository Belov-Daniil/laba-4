...
count_zero_e_even = 0  # количество нулевых элементов в четных столбцах
count_zero_e_odd = 0  # количество нулевых элементов в нечетных столбцах
...
if  count_zero_e_even > count_zero_e_odd:
    print('Меняем местами С и B симметрично')
    if N % 2 == 0:
    ...
    print('Меняем местами B и C несимметрично')
    if N % 2 == 0:
        F[0:w, 0:w] = B
        F[w:, w:] = E
    else:
...
if det_A > sum_diag:  
    ...
    result = np.linalg.matrix_power(A, -1) * A.T - K*F
else:
    if np.linalg.det(F) == 0:
        ...
        result = (A.T + G-np.linalg.matrix_power(F, -1))*K
